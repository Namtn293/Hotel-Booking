...
import project.backend.hotel_booking.repository.ImageRepositoty;
...
public class ImageServiceImplement implements ImageService {
    ...
    public ImageServiceImplement(Cloudinary cloudinary, ImageRepositoty imageRepositoty) {
    ...
    public void deleteImage(Long imageId) {
        try {
            Image image = imageRepositoty.findById(imageId)
                    .orElseThrow(()->new BusinessException(ErrorCode.IMAGE_NOT_EXIST));
            Map result = cloudinary.uploader().destroy(
                    image.getPublicId(),
                    ObjectUtils.emptyMap()
            );
            imageRepositoty.delete(image);
        }catch (Exception e){
